...
<?php
    ...
    $sql = $koneksi->query("SELECT * FROM tb_laundry WHERE id_laundry= '$kode'");
    ...
    if (isset($_POST['simpan'])) {
        ...
        $sql2 = $koneksi->query("UPDATE tb_laundry (id_pelanggan, kd_user, tanggal_terima, tanggal_selesai, jml_kiloan, totalbayar, status, catatan, kd_jenis) VALUES ('$pelanggan','$id_user','$tanggal_terima','$tanggal_selesai','$jml_kiloan','$totalbayar','$status','$catatan','$jenis')");
        ...
        if ($status=="Lunas") {
            $sql2 = $koneksi->query("UPDATE tb_transaksi (kd_user, tgl_transaksi, pemasukan, catatan, keterangan)values('$id_user','$tanggal_selesai','$totalbayar','$catatan','pemasukan')");
