...
$result = mysqli_query($koneksi, "SELECT * FROM tb_jenis WHERE kd_jenis = '$id'");
...
if (isset($_POST['ubah'])) {
  ...
  if ($row['jenis_laundry'] !== $jenis_laundry) {
    ...
    $query_jenis = mysqli_query($koneksi, "SELECT * FROM tb_jenis WHERE jenis_laundry = '$jenis_laundry'");
  ...
  if ($pesan_error == "") {
    $query = mysqli_query($koneksi, "UPDATE `tb_jenis` SET `jenis_laundry` = '$jenis_laundry', `lama_proses` = '$lama_proses', `tarif` = '$tarif' WHERE `tb_jenis`.`kd_jenis` = $id");
    if ($query) {
      echo "
      <script>
      ...
      <?php endif; ?>
          ...
          <div class="card m-b-100">
            <div class="card-body">
              ...
              <div class="form-group row">
                <label for="example-text-input" class="col-sm-2 col-form-label">Tarif (Satuan / Per Kg)</label>
                <div class="col-sm-10">
